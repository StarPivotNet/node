...
function getFormatOfExtensionlessFile(url) {
  ...
  if (uclawnpm.isArchivePath(path)) {
    const source = uclawnpm.readFileSync(path);
    return source.length >= 4 && source[0] === 0x00 && source[1] === 0x61 &&
      source[2] === 0x73 && source[3] === 0x6d ? 'wasm' : 'module';
  }
