...
function legacyMainResolve(packageJSONUrl, packageConfig, base) {
  ...
  if (uclawnpm.isArchivePath(pkgPath)) {
    const main = packageConfig.main || '';
    const candidates = main === '' ?
      ['index.js', 'index.json'] :
      [main, `${main}.js`, `${main}.json`, `${main}/index.js`, `${main}/index.json`, 'index.js', 'index.json'];
    for (let i = 0; i < candidates.length; i++) {
      const candidate = resolve(pkgPath, candidates[i]);
      if (uclawnpm.stat(candidate) === 0) {
...
function finalizeResolution(resolved, base, preserveSymlinks) {
  ...
  const normalizedPath = StringPrototypeEndsWith(path, '/') ? StringPrototypeSlice(path, -1) : path;
  const archiveStats = uclawnpm.stat(normalizedPath);
  const stats = archiveStats >= 0 ? archiveStats :
    internalFsBinding.internalModuleStat(normalizedPath);
  ...
  if (!preserveSymlinks && !uclawnpm.isArchivePath(path)) {
