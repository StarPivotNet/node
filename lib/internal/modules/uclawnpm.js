...
function getModuleRoots() {
  ...
  const mediaRoot = safeGetenv('UCLAW_OPENCLAW_MEDIA_ROOT');
  const cacheRoot = safeGetenv('UCLAW_OPENCLAW_MODULE_CACHE_ROOT');
  moduleRoots = mediaRoot && cacheRoot && path.isAbsolute(mediaRoot) && path.isAbsolute(cacheRoot) ?
    { __proto__: null, mediaRoot: path.resolve(mediaRoot), cacheRoot: path.resolve(cacheRoot) } : null;
  return moduleRoots;
}

function mapModulePathToCache(filename) {
  ...
  const relative = path.relative(roots.mediaRoot, filename);
  if (relative === '' || (!path.isAbsolute(relative) && relative !== '..' &&
      !StringPrototypeStartsWith(relative, `..${path.sep}`))) {
    return path.join(roots.cacheRoot, relative);
  }
}
